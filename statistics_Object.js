...
/*--Creating Senate At A glance table with party values --*/
function getSenateAtAGlancetable(members) {
  ...
  /*---Calculate the average with party percentage values---*/
  ...
  /*---Create 3 lists of Republicans,Democrats and Independents---*/
  for (i = 0; i < party.length; i++) {
    if (party[i] === "D") democrats.push(party[i]);
  }
  ...
  sumOfReps =
    statistcs.numOFDemocrats +
    statistcs.numOFReplublicans +
    statistcs.numOFIndependents;

  tableParty(
    "Democrats",
    statistcs.numOFDemocrats,
    statistcs.avgOfDemocrats.toFixed(2)
  );
  tableParty(
    "Replublicans",
    statistcs.numOFReplublicans,
    statistcs.avgOfRepublicans.toFixed(2)
  );
  tableParty(
    "Independents",
    statistcs.numOFIndependents,
    statistcs.avgOfIndependents.toFixed(2)
  );
...
function getEngageTable(members) {
  ...
  for (let n = 0; n < leastEngaged.length * 0.1; n++) {
    ...
    console.log(
      "TOP TEN LIST" +
        statistcs.least_Engaged_pct[n].missed_votes_pct +
        statistcs.least_Engaged_pct[n].first_name +
        statistcs.least_Engaged_pct[n].missed_votes
    );
...
function getLoyaltyTable() {
  ...
  for (let i = 0; i < leastLoyal.length; i++) {
    ...
    console.log(
      "DESCENDING ORDER ARRAY" + leastLoyal[i].votes_against_party_pct
    );
  }
  for (let n = 0; n < leastLoyal.length * 0.1; n++) {
    ...
    console.log(
      "TOP TEN LIST" +
        statistcs.least_Loyal_pct[n].first_name +
        statistcs.least_Loyal_pct[n].total_votes +
        statistcs.least_Loyal_pct[n].votes_against_party_pct
    );
  ...
  for (n = 0; n < mostLoyal.length * 0.1; n++) {
    ...
    console.log(
      "BOTTOM ten" + statistcs.most_Loyal_pct[n].votes_against_party_pct
    );
