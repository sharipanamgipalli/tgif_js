...
/*--Sending the request with respective urls and API key--*/
...
/*---Getting the JSON response sending url and request containing header--*/
...
fetch(url, request)
  ...
  .then(myjson => {
    ...
    /*---Implement both checkbox and dropdown filters on each of the parties--*/
    if (
      document.title.includes("FILTER") ||
      document.title.includes("FILTER")
    ) {
      document
        .getElementById("democrats")
      ...
      /*---Creating the table and dropdown list with members--- */
      addDataToTable(members);
      createDropDownList(members);
    }
    /*---Creating the senate at a glance and Least & Most Enaged tables for Senate and House pages ---*/

    if (document.title.includes("Attendance")) {
      getSenateAtAGlancetable(members);
      getEngageTable(members);
    }
    /*---Creating the senate at a glance and Least & Most Loyal tables for Senate and House pages ---*/
...
/*---Filter by checkbox and dropdown values----*/
function getfilteredListByBoth(selectedState) {
  ...
  for (i = 0; i < members.length; i++) {
    if (checked_values.length === 0 && selectedState === "All") {
      ...
      checked_values.includes(members[i].party) &&
      selectedState === "All"
    ) {
      filteredMembersList.push(members[i]);
    } else if (
      checked_values.length === 0 &&
      selectedState === members[i].state
    ) {
      filteredMembersList.push(members[i]);
    } else if (
      checked_values.includes(members[i].party) &&
